export type GenericFormState<T> =
	{ tag: "editing"; error?: string; inputs: T } | { tag: "submitting"; inputs: T } | { tag: "redirect" };
...
export function genericFormReducer<T>(state: GenericFormState<T>, action: GenericFormAction<T>): GenericFormState<T> {
	switch (state.tag) {
		case "editing":
			if (action.type === "edit") {
				return {
