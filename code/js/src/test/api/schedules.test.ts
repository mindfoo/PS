...
const baseSched = {
  id: 's1', workflowId: 'wf1', workflowName: 'Pipeline',
  cronExpression: '0 30 9 * * *', timezone: 'UTC',
  enabled: true, nextRunAt: '2026-06-01T09:30:00', lastRunAt: null, description: null,
}
...
describe('api/schedules', () => {
  ...
  it('create sends POST and returns created schedule', async () => {
    ...
    const result = await scheduleApi.create({
      workflowId: 'wf1', cronExpression: '0 30 9 * * *', timezone: 'UTC',
    })
  ...
  it('update sends PUT and returns updated schedule', async () => {
    ...
    const result = await scheduleApi.update('s1', {
      cronExpression: '0 0 10 * * *', timezone: 'UTC', enabled: true,
    })
  ...
  it('create throws on 400 invalid cron', async () => {
    ...
    const { scheduleApi } = await import('../../api/schedules')
    await expect(scheduleApi.create({
      workflowId: 'wf1', cronExpression: 'INVALID',
    })).rejects.toThrow()
  })
})
