...
export function WorkflowDetailPage() {
  ...
  // Holds the unsubscribe cleanup function
  const activeSseRef = useRef<(() => void) | null>(null)
  ...
  useEffect(() => {
    return () => {
      if (activeSseRef.current) {
        activeSseRef.current()
      }
    }
  }, [])

  // --- SSE Tracking Monitor ---
  const subscribeAndMonitor = useCallback((execId: string) => {
    // Gracefully clean up previous open connections if they exist
    if (activeSseRef.current) {
      activeSseRef.current()
      activeSseRef.current = null
    }

    activeSseRef.current = executionApi.subscribeToExecution(execId, (event) => {
                                        ...
                                        {perms.canWriteTasks ? (
                                            editingStage === t.taskId ? (
                                                <input
                                                    ...
                                                    onKeyDown={e => {
                                                      if (e.key === 'Enter') commitStageEdit(t.taskId);
                                        ...
                                        {perms.canWriteTasks ? (
                                            editingRetry === t.taskId ? (
                                                <input
                                                    ...
                                                    onKeyDown={e => {
                                                      if (e.key === 'Enter') void commitRetryEdit(t.taskId);
                                                      if (e.key === 'Escape') setEditingRetry(null)
                                                    }}
                                                />
                                            ) : (
                                                <span
            ...
            <div>
              ...
              {executions.length > 0 && (
                ...
                <span className="badge badge-muted">
                  ...
                  {executions.filter(e => e.triggeredType === 'MANUAL').length === 1 ? 'run' : 'runs'}
                </span>
                    {executions.some(e => e.triggeredType === 'CRON') && (
                        <span className="badge badge-muted">
                    {executions.filter(e => e.triggeredType === 'CRON').length} cron{' '}
                          {executions.filter(e => e.triggeredType === 'CRON').length === 1 ? 'run' : 'runs'}
                  </span>
                    )}
                  </div>
              )}
                        ...
                        <table className="table">
                          ...
                          {executions.map(ex => (
                              <tr key={ex.id}>
                                ...
                                <td className="actions-cell">
                                  <Link to={`/workflows/${id}/executions/${ex.id}`} className="btn btn-sm btn-secondary">Details</Link>
                                  {(ex.status === 'PENDING' || ex.status === 'RUNNING') && perms.canExecuteWorkflows && (
                                      <button className="btn btn-sm btn-danger" onClick={() => void handleCancelExecution(ex.id)}>Cancel</button>
                                  )}
                                </td>
                              </tr>
                          ))}
