...
export function DashboardPage() {
	...
	async function handleRun(id: string) {
		...
		try {
			...
			navigate(`/workflows/${id}/executions/${res.executionId}`);
		} catch (err: unknown) {
					...
					{workflows.map((w) => (
						<div key={w.id} className="card">
							...
							<div className="card-actions">
								<Link to={`/workflows/${w.id}`} className="btn btn-sm btn-secondary">
									View
								</Link>
								{perms.canWriteWorkflows && (
									<Link to={`/workflows/${w.id}/edit`} className="btn btn-sm btn-secondary">
										Edit
									</Link>
								)}
								{perms.canExecuteWorkflows && (
									<button className="btn btn-sm btn-success" onClick={() => handleRun(w.id)}>
										▶ Run
									</button>
								)}
