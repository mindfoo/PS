...
export function Layout({ children }: { children: ReactNode }) {
	...
	return (
		<div className={`app-layout${collapsed ? " app-layout--sidebar-collapsed" : ""}`}>
			...
			<button className="hamburger-btn" aria-label="Toggle navigation" onClick={() => setMenuOpen((o) => !o)}>
				{menuOpen ? "✕" : "☰"}
			</button>
			...
			<nav className={sidebarClass} aria-label="Main navigation">
				...
				<div className="sidebar-footer">
					...
					<button onClick={handleLogout} className="btn btn-ghost btn-sm nav-label">
						Logout
					</button>
				</div>
